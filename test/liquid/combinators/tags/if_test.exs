defmodule Liquid.Combinators.Tags.IfTest do
  ...
  test "if else " do
    test_combinator(
      ...
      &Parser.if/1,
      if: [evaluation: ["foo"], body: [" YES ", {:else, [" NO "]}]]
    )
  end
  ...
  test "nested if" do
    test_combinator(
      ...
      if: [
        evaluation: [false],
        body: [if: [evaluation: [false], body: [" NO "]]]
      ]
    )
  ...
  test "comparing values" do
    test_combinator(
      "{% if null < 10 %} NO {% endif %}",
      &Parser.if/1,
      if: [evaluation: [condition: {nil, :<, 10}], body: [" NO "]]
    )

    test_combinator(
      "{% if 10 < null %} NO {% endif %}",
      &Parser.if/1,
      if: [evaluation: [condition: {10, :<, nil}], body: [" NO "]]
    )
  end
  ...
  test "usisng contains" do
    test_combinator(
      ...
      &Parser.if/1,
      if: [
        evaluation: [condition: {"bob", :contains, "f"}],
        body: ["yes", {:else, ["no"]}]
      ]
    )
  end
